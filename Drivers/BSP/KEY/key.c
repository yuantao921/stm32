...
void key_init(void)
{
    ...
    gpio_init_struct.Pin = WKUP_GPIO_PIN;                       /* WKUP引脚（KEY3） */
    gpio_init_struct.Mode = GPIO_MODE_INPUT;                    /* 输入 */
    gpio_init_struct.Pull = GPIO_PULLUP;                       /* 上拉 */
    gpio_init_struct.Speed = GPIO_SPEED_FREQ_HIGH;              /* 高速 */
    HAL_GPIO_Init(WKUP_GPIO_PORT, &gpio_init_struct);           /* WKUP引脚模式设置,上拉输入 */
...
uint8_t key_scan(uint8_t mode)
{
    ...
    if (key_up && (KEY0 == 0 || KEY1 == 0 || KEY2 == 0 || WK_UP == 0 || KEY4 == 0))  /* 按键松开标志为1, 且有任意一个按键按下了 */
    {
        delay_ms(10);           /* 去抖动 */
        key_up = 0;
        ...
        if (WK_UP == 0) keyval = WKUP_PRES;  /* KEY3/WKUP（上拉模式：按下为0） */

        if (KEY4 == 0)  keyval = KEY4_PRES;
    }
    else if (KEY0 == 1 && KEY1 == 1 && KEY2 == 1 && WK_UP == 1 && KEY4 == 1)         /* 没有任何按键按下, 标记按键松开 */
    {
        key_up = 1;
    }
